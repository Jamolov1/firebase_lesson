...
import '../details/shirinlik_detail_page.dart';

class ShirinliklarPage extends StatefulWidget {
  const ShirinliklarPage({super.key});

  @override
  State<ShirinliklarPage> createState() => _ShirinliklarPageState();
}

class _ShirinliklarPageState extends State<ShirinliklarPage> {
  List<Post> items = [];
  ...
  void deleteIchimlik(String id) {
    RTDBService.deleteShirinliklar(id).then((value) => {_apiPostList()});
  }
  ...
  void deleteShirinlik(String id) {
    RTDBService.deleteShirinliklar(id).then((value) => {_apiPostList()});
  }
